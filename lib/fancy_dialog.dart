...
class FancyDialog extends StatefulWidget {
  ...
  const FancyDialog({Key key, this.title, this.descreption, this.okFun,this.cancelFun,
  
  this.animationType=1,this.gifPath=FancyGif.MOVE_FORWARD,this.cancelColor=Colors.grey,
  this.okColor=Colors.pink,this.ok=" OK !",this.cancel="Cancel",this.theme=0}): super(key: key);
...
class GifDialogState extends State<FancyDialog> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    if(gifPath=='CHECK_MAIL'){gifPath='assets/gif3.gif';package=1;}
    else if(gifPath=='FUNNY_MAN'){gifPath='assets/gif1.gif';package=1;}
    else if(gifPath=='MOVE_FORWARD'){gifPath='assets/gif4.gif';package=1;}
    else if(gifPath=='PLAY_MEDIA'){gifPath='assets/gif2.gif';package=1;}
    else if(gifPath=='SUBMIT'){gifPath='assets/gif5.gif';package=1;}
    else if(gifPath=='SHARE'){gifPath='assets/gif6.gif';package=1;}
    double start;
    int animationType = widget.animationType;
      switch(animationType){
        case 1 : {start=-1.0;break;}
        case -1 : {start=1.0;break;}
  ...
  Widget build(BuildContext context) {
    ...
    if(package==1){
      image =ClipRRect(child: Image.asset(gifPath,fit: BoxFit.fill,width: dwidth,package:'fancy_dialog',),
              borderRadius:BorderRadius.only(topLeft: Radius.circular(theme==0 ? 15:0),topRight: Radius.circular(theme==0 ? 15:0)),
              );
    }else {
      image=ClipRRect(child: Image.asset(gifPath,fit: BoxFit.fill,width: dwidth,),
              borderRadius:BorderRadius.only(topLeft: Radius.circular(theme==0 ? 15:0),topRight: Radius.circular(theme==0 ? 15:0)),
              );
    }
        ...
        child: Container(
          ...
          Container(
              ...
              children: <Widget>[
                ...
                Padding(padding: EdgeInsets.only(left: theme==0 ? 0.07*dwidth:1),
                child: theme==0 ? customButton(ok, okColor, okFun):flatButton(ok, okColor, okFun),
                )
              ],
            ),
            )
